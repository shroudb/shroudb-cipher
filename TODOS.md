...
- [ ] **DEBT-F-cipher-8** — cipher engine must normalize `actor: None` to `"anonymous"` before emitting audit events. Currently passes empty string, which Chronicle validation rejects; `shroudb-moat` works around this with an `EventNormalizer` at the audit boundary. Upstream fix is a 1-line `actor.unwrap_or("anonymous")` in `emit_audit_event`. *No debt test yet; add one before fixing.*
- [ ] **DEBT-F-cipher-9** — `rewrap`, `generate_data_key`, `verify_signature` must become `async fn` and route through Sentry on the hot path, matching `encrypt`/`decrypt`/`sign` (closes the remaining gap from DEBT-F-cipher-3). Cross-repo API break: cascades through `shroudb-moat/src/engine/{sigil,scroll,stash,courier}.rs`, `shroudb-sigil-server/src/cipher_embedded.rs`, `shroudb-stash-server/src/cipher_embedded.rs`, `shroudb-scroll-server/src/cipher_embedded.rs`. *No debt test yet; add one before fixing. Coordinate cross-repo.*
