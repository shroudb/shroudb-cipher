...
Encryption-as-a-service engine for ShrouDB.

## Identity

Cipher is an **encryption key lifecycle manager and cryptographic operations server**. Applications send plaintext to Cipher and receive ciphertext; keys never leave the server. Cipher manages keyrings of versioned keys with automatic rotation, draining, and retirement.
...
- **Audit surface changes require scrutiny.** Any change that modifies authentication, authorization, key management, encryption operations, or network-facing code must be reviewed with the assumption that an attacker will examine it.
...
```
shroudb-cipher-core/        — domain types (Keyring, KeyVersion, CiphertextEnvelope, etc.)
shroudb-cipher-engine/      — Store-backed logic (CipherEngine, KeyringManager, crypto ops)
shroudb-cipher-protocol/    — RESP3 command parsing + dispatch (Moat integration path)
shroudb-cipher-server/      — TCP binary (standalone deployment)
...
- **Downstream:** shroudb-moat (embeds engine + protocol), shroudb-codegen (reads `protocol.toml`), shroudb-sigil (uses Cipher via CipherOps capability trait), shroudb-veil (uses Cipher for blind index derivation)
