...
/// TCP connection to a Cipher server speaking RESP3.
pub struct Connection(shroudb_client_common::Connection);

impl Connection {
    /// Connect directly to a standalone Cipher server.
    ...
    /// Connect to a Cipher engine through a Moat gateway.
    pub async fn connect_moat(addr: &str) -> Result<Self, ClientError> {
        Ok(Self(
            shroudb_client_common::Connection::connect_with_prefix(addr, "CIPHER").await?,
        ))
    }
